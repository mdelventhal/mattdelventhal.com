...
The problem as specified above is already nearly ready to be introduced to the Gurobi solver. All that remains is to define some auxiliary variables:
  ...
  - $\theta_{ikr} \equiv  \left \\{ \begin{array}{r l}\left ( \sum\limits_{j = 1}^J  X_{ijk,r+1}^0  \right ) \left (x_{ikr} - \sum\limits_{j=1}^J X_{ijkr}^1  \right )   & {\small \text{ for } r \in \\{1, 2, ..., R-1 \\} } \\\\ \\\\ x_{ikr} & {\small \text{ for } r = R  }  \end{array} \right.$
