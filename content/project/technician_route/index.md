...
The company's problem can then formally be given as $$ \min\limits_{x_{jkr} ~ , ~ t_j} \left \\{   \sum\limits_{j=1}^J \pi_j \times \left \[  \left ( 1 - z_j  \right ) ~ \cdot \overbrace{C_1}^{\begin{array}{c}\text{penalty} \\\\ \text{for} \\\\ \text{no-show} \end{array}}   + ~ z_j \cdot \bigg \[ \underbrace{\max \\{ 0 ~ , ~ t_j + p_j - c_j \\}}\_{\begin{array}{c} \text{lateness} \end{array}} + ~ C_0 \cdot \underbrace{\max \\{ 0 ~ , ~ a_j - t_j \\}}\_{\begin{array}{c} \text{start} \\\\ \text{too early} \end{array}} + ~ C_0 \cdot \underbrace{ \max \\{ 0 ~ , ~ t_j - b_j \\}}\_{\begin{array}{c} \text{start} \\\\ \text{too late} \end{array}} \bigg \] \right \] \right \\} $$
