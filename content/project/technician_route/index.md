...
4. **Job starts allow sufficient time to arrive at job site from previous location:** $$ t_j \leq \sum\limits_{k = 1}^K x_{jk0} \tau_{ \small O_k,L_j} \\\\ \quad \quad + \sum\limits_{r=2}^J \sum\limits_{i = 1}^J \sum\limits_{k=1}^K x_{jkr} x_{ik,r-1} \left \[ t_i + p_i + \tau_{\small L_i,L_j}  \right \] {\small \text{ for } j \in \\{1,2,...,J \\} }$$
5. **No technician works more than time allocation:** $$ \begin{aligned}\sum\limits_{r = 1}^J \sum\limits_{j = 1}^{J} x_{jkr} p_j \\\\
6. \quad \quad + \sum\limits_{r=2}^{J} \sum\limits_{i=1}^J \sum\limits{j=1}^J x_{ik,r-1} x_{jk,r} \tau_{\small L_i, L_j} \\\\ \leq w_k {\small \text{ for } k \in \\{1, 2, ..., K \\} } \end{aligned}$$
