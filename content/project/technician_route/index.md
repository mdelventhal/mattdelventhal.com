...
The company's problem can then formally be given as $$ \min\limits_{x_{jkr},t_j} \left \\{   \sum\limits_{j=1}^J \pi_j \times \left \[  \left ( 1 - \sum\limits_{r = 1}^R \sum\limits_{k = 1}^{K} x_{jkr}  \right ) \cdot C_1 \right \] \right \\} $$
