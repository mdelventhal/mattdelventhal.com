...
3. **Job starts allow sufficient time to arrive at job site from previous location:** $$ t_j \leq \sum\limits_{k = 1}^K x_{jk0} \tau_{O_k,L_j} + \sum\limits_{r=2}^J \sum\limits_{i = 1}^J \sum\limits_{k=1}^K x_{jkr} x_{ik,r-1} \left \[ t_i + p_i + \tau_{\tiny L_i,L_j}  \right \] $$
